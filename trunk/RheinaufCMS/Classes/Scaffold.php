...
<table>
<thead>
<tr><td colspan=\"$colspan_minus1\">{page_browser}<td><td><a href=\"{SELF_URL}?export&amp;allresults&amp;{If:export_get_vars}\"><img src=\"/RheinaufCMS/Classes/Scaffold/icon_excel_doc.gif\" alt=\"Excel\" title=\"Ergebnisse als Excel-Tabelle speichern\" /></a> </td></tr>
...
";
		...
		foreach ($this->cols_array as $key => $col)
		{
			...
			if ($name != 'id')
			{
				...
				if ($col['info'])
				{
					...
					$info = ' '.Html::img('/'.INSTALL_PATH . "/Classes/Scaffold/info.gif",strip_tags($col['info']),array('id' =>$trigger_id,'title'=>strip_tags($col['info'])));
	...
	function delete_entry()
	{
		...
		$this->connection->db_query("DELETE FROM `$this->table` WHERE `id` = ".$_GET['delete']);
...
//		header("Location: $url");
	...
	function get_options($options_value,$sort = false,$insert_index = false)
	{
		...
		{
			$options = array();
			$a = $this->connection->db_assoc("SELECT * FROM `$this->table>Options` WHERE `Context` = '$options_value' ORDER BY `id`");
			foreach ($a as  $v)
			{
	...
	function edit_options()
	{
		...
		$edit_options_scaff = new Scaffold($this->table.'>Options',$this->connection);
