...
RheinaufCMSLinker._pluginInfo =
{
  name     : "RheinaufCMSLinker",
  version  : "1.0",
...
function RheinaufCMSLinker(editor, args)
{
  this.editor  = editor;
  this.lConfig = editor.config.RheinaufCMSLinker;
...
RheinaufCMSLinker.prototype._lc = function(string)
{
  return HTMLArea._lc(string, 'RheinaufCMSLinker');
}
...
// Inline Dialog for RheinaufCMSLinker

RheinaufCMSLinker.Dialog_dTrees = [ ];


RheinaufCMSLinker.Dialog = function (linker)
{
  var  lDialog = this;
  this.Dialog_nxtid = 0;
  this.linker = linker;
...
RheinaufCMSLinker.Dialog.prototype._prepareDialog = function()
{
  ...
  {
    HTMLArea._loadback(_editor_url + 'plugins/RheinaufCMSLinker/dTree/dtree.js',
  ...
  if(this.html == false)
  {
    HTMLArea._getback(_editor_url + 'plugins/RheinaufCMSLinker/dialog.html', function(txt) { lDialog.html = txt; lDialog._prepareDialog(); });
    return;
  }
  var html = this.html;

  // Now we have everything we need, so we can build the dialog.
  var dialog = this.dialog = new HTMLArea.Dialog(linker.editor, this.html, 'RheinaufCMSLinker');
  var dTreeName = HTMLArea.uniq('dTree_');

  this.dTree = new dTree(dTreeName, _editor_url + 'plugins/RheinaufCMSLinker/dTree/');
...
RheinaufCMSLinker.Dialog.prototype.makeNodes = function(files, parent)
{
  for(var i = 0; i < files.length; i++)
  {
    if(typeof files[i] == 'string')
    {
      this.dTree.add(RheinaufCMSLinker.nxtid++, parent,
                     files[i].replace(/^.*\//, ''),
//'javascript:document.getElementsByName(\'' + this.dialog.id.href + '\')[0].value=decodeURIComponent(\'' + encodeURIComponent(files[i]) + '\');document.getElementsByName(\'' + this.dialog.id.type + '\')[0].click();document.getElementsByName(\'' + this.dialog.id.href + '\')[0].focus();void(0);',
 //'javascript:document.getElementsByName(\'' + this.dialog.id.href + '\')[0].value=\'' + files[i] + '\';document.getElementsByName(\'' + this.dialog.id.type + '\')[0].click();document.getElementsByName(\'' + this.dialog.id.href + '\')[0].focus();void(0);',
...
//if(files[i].url) var link = 'javascript:document.getElementsByName(\'' + this.dialog.id.href + '\')[0].value=\'' + files[i].url + '\';document.getElementsByName(\'' + this.dialog.id.type + '\')[0].click();document.getElementsByName(\'' + this.dialog.id.href + '\')[0].focus();void(0);';
...
RheinaufCMSLinker.Dialog.prototype.show = function(inputs, ok, cancel)
{
  if(!this.ready)
  {
    var lDialog = this;
