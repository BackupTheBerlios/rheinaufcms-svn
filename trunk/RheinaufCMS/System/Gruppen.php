...
class Gruppen extends Admin
{
	...
	function Gruppen(&$system)
	{
		...
		$this->rechte = $this->connection->db_assoc("SELECT * FROM `RheinaufCMS>Rechte` ORDER BY `Frontend_Backend`,  `ModulName` ASC, `RechtName` ASC ");
	...
	{
		$this->existent_groups = General::multi_unserialize($this->connection->db_assoc("SELECT * FROM `RheinaufCMS>Groups`"));
	}
	...
	function rechte_update()
	{
		...
		$this->connection->db_query("UPDATE `RheinaufCMS>Groups` SET `Name` = '$name', `Rechte` = '$new_rechte' WHERE `id` = '$id'");
		$this->connection->db_query("UPDATE `RheinaufCMS>User` SET `Group` = '$name' WHERE `Group` = '$old_name'");
	...
	function new_group_input()
	{
		...
		$this->connection->db_query("INSERT INTO `RheinaufCMS>Groups` ( `id` , `Name` , `Rechte` ) VALUES ('$id', '$name', '$new_rechte')");
		$this->group_table_update();
	}
	...
	{
		$id = $_GET['deletegroup'];
		$this->connection->db_query("DELETE FROM `RheinaufCMS>Groups` WHERE `id` = $id");
		$this->group_table_update();
	}
}
?>