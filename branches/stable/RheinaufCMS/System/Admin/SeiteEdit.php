...
class SeiteEdit extends Admin
{
	...
	{
		$this->navi = $this->system->navi_array();
		$this->linker_navi();
	}
	...
	function save()
	{
		...
		if (RheinaufFile::is_file($folder.'tmp.html')) RheinaufFile::unlink($folder.'tmp.html');
		$files = RheinaufFile::dir_array($folder,false,'html');
		rsort($files,SORT_NUMERIC);
	...
	function revert ()
	{
		...
		if ($file == 'tmp.html') RheinaufFile::unlink ($folder.$file);
	}
	...
	function golive ()
	{
		...
		$folder = INSTALL_PATH . "/Content/$rubrik/$seite/";
		if (RheinaufFile::is_file($folder.'Arbeitsversion/content.html')) RheinaufFile::copy($folder.'Arbeitsversion/content.html',$folder.'content.html');
	}
