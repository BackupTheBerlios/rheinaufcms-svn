...
class Navi extends RheinaufCMS
{
	...
	var $accesskey = 0;
	var $navi;
	function Navi(&$system)
	{
		$this->system =& $system;
		$this->navi =& $system->navi;
			...
			switch ($this->navi[$i]['Rubrik'])
			{
				...
				default:
					if ($this->navi[$i]['Show'] == '1' )
					{
						...
						if ( $this->navi[$i]['Rubrik'] == HOMEPAGE )
						{
							$vars['link'] = '/';
						}
						else if ($this->navi[$i]['ext_link'] != '')
						{
							$vars['link'] = $this->navi[$i]['ext_link'];
						}
						else $vars['link'] = '/'. rawurlencode($this->path_encode($this->navi[$i]['Rubrik'])).'/';
						$vars['navi_class'] =  ($this->navi[$i]['Rubrik'] == $this->system->rubrik) ? 'navi navi_active' : 'navi';


						$navi_string .= $this->template->parse_template('RUBRIK',$vars);
						if ($this->navi[$i]['Rubrik'] == $this->system->rubrik)
						{
	...
	function sub_navi($i)
	{
		...
		for ($j = 0;$j<count($subnavi);$j++)
		{
			if ($subnavi[$j]['Show'] == '1' && $subnavi[$j]['Seite'] != 'index')
			{
				...
				$vars['subnavi_class'] = ($subnavi[$j]['Seite'] == $this->system->seite) ? 'subnavi subnavi_active' : 'subnavi';
