...
class Navi extends RheinaufCMS 
{
	...
	function rubriken ()
	{
			...
			switch ($this->navi[$i]['Rubrik'])
			{
				...
				default:
					if ($this->navi[$i]['Show'] == '1' )
					{
						...
						$vars['link'] = $this->homepath . '/'. rawurlencode($this->path_encode($this->navi[$i]['Rubrik'])).'/';
		...
		$navi_string .= $this->template->get_part('POST')."\n";
	...
	function sub_navi($i)
	{
		...
		{
			if ($subnavi[$j]['ext_link'] == '' &&  $subnavi[$j]['Show'] == '1' && $subnavi[$j]['Seite'] != 'index') 
			{
				$vars['tabindex'] = $this->tabindex;
				$this->tabindex++;
				$vars['accesskey'] = $this->accesskey_array[$this->accesskey];
